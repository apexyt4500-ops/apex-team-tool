...
def main():
    ...
    while True:
        print_input_prompt("Enter file path (e.g., /sdcard/lib.so):")
    ...
    print_input_prompt("Enter XOR key (decimal, 0 for none):")
    xor_input = input().strip()
    try:
    ...
    # Choose URL
    while True:
        print_input_prompt("Enter URL number to replace (or 0 to quit):")
        choice = input().strip()
        if choice == '0':
    ...
    # New URL
    print_input_prompt("Enter new URL:")
    new_url = input().strip()
    if not new_url:
